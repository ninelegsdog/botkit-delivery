...
async def test_create_order(db):
    ...
    order_id = await service.create_order(
        db, number="ORD-001", title="Test order", status_id=int(statuses[0]["id"])
    )
    assert order_id > 0
...
async def test_get_order_by_number(db):
    ...
    await service.create_order(
        db, number="ORD-002", title="Test order", status_id=int(statuses[0]["id"])
    )
...
async def test_update_order_status(db):
    statuses = await service.get_statuses(db)
    order_id = await service.create_order(
        db, number="ORD-003", title="Test order", status_id=int(statuses[0]["id"])
    )
...
async def test_get_order_history(db):
    statuses = await service.get_statuses(db)
    order_id = await service.create_order(
        db, number="ORD-004", title="Test order", status_id=int(statuses[0]["id"])
    )
...
async def test_subscribe_order(db):
    statuses = await service.get_statuses(db)
    order_id = await service.create_order(
        db, number="ORD-005", title="Test order", status_id=int(statuses[0]["id"])
    )
...
async def test_unsubscribe_order(db):
    statuses = await service.get_statuses(db)
    order_id = await service.create_order(
        db, number="ORD-006", title="Test order", status_id=int(statuses[0]["id"])
    )
    await service.subscribe_order(db, order_id, 123)
...
async def test_close_order(db):
    statuses = await service.get_statuses(db)
    order_id = await service.create_order(
        db, number="ORD-007", title="Test order", status_id=int(statuses[0]["id"])
    )
...
async def test_get_order_count(db):
    statuses = await service.get_statuses(db)
    await service.create_order(
        db, number="ORD-008", title="Test order", status_id=int(statuses[0]["id"])
    )
...
async def test_get_status_stats(db):
    statuses = await service.get_statuses(db)
    await service.create_order(
        db, number="ORD-009", title="Test order", status_id=int(statuses[0]["id"])
    )
